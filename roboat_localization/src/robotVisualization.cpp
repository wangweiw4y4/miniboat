...
public:
  RobotVisualiztion() : dx(0), dy(0)
  {
    pub_path = nh.advertise<nav_msgs::Path>("visual/path", 1);
    pub_marker = nh.advertise<visualization_msgs::Marker>("visual/robot", 1);
    pub_force_marker = nh.advertise<visualization_msgs::Marker>("visual/hud/force", 1);
    pub_priority_marker = nh.advertise<visualization_msgs::Marker>("visual/hud/priority", 1);

    sub_priority = nh.subscribe("thrust_state", 1, &RobotVisualiztion::thrustStateCallback, this);
    sub_force = nh.subscribe("force", 1, &RobotVisualiztion::forceCallback, this);
    current_odometry = nh.subscribe("state", 1, &RobotVisualiztion::odometryCallback, this);

    path_update_timer = nh.createTimer(ros::Duration(update_rate), &RobotVisualiztion::pathHandler, this);
  }
...
{
  ros::init(argc, argv, "roboat_localization");

  RobotVisualiztion RV;

  ROS_INFO("\033[1;32m----> roboat_localization Robot Visualization.\033[0m");
