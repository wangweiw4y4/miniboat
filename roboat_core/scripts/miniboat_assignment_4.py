...
class Test:
    def __init__(self):
        ...
        self.latch = UInt16()
        self.counter_reset = UInt16()

        '''
        rospy.Subscriber("/miniboat5/odometry/filtered", Odometry, self.mb1_callback)
        rospy.Subscriber("/miniboat6/odometry/filtered", Odometry, self.mb2_callback)
        ...
        self.l_mb2_pub = rospy.Publisher("/miniboat6/latching", UInt16, queue_size=10)
    ...
    def counter_publish(self, _reset):
        self.counter_reset.data = _reset
        self.admm_restart_pub.publish(self.counter_reset)

def main():
    rospy.init_node('miniboat_assignment_4', anonymous=False)
    freq = 50
    ...
    while (not rospy.is_shutdown()):
        ...
        if state == 2:
            ...
            t.latch_action(2)
            t.flag = 1.0
            while (not rospy.is_shutdown() and (state == 2)):
                if t.flag == 1.0:
                    #rospy.logwarn("Square Assignment")
                    robot_poses = np.array([[t.pose_mb1.x,t.pose_mb1.y],[t.pose_mb2.x,t.pose_mb2.y],[t.pose_mb3.x,t.pose_mb3.y],[t.pose_mb4.x,t.pose_mb4.y]])
                    for i in range(number_of_robots):
