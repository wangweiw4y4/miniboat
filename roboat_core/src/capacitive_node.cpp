...
bool allCalibrated(bool temp_c[4])
{ // function to check if all the sensors have been calibrated.

    for (int i = 0; i < 4; i++)
    {
        if (temp_c[i] == true)
        { // i.e. if any are still calibrating (true), break and return false (i.e. we haven't calibrated all of them).
            return false;
            break;
        }
    }
...
int latched(int i)
{ // function to check if we are latched.
    int l = 0;
    if (cap_baseline[i] - cap_temp[i] > threshold)
    { // compare against the calibrated baseline
        l = 1;
    }
    else
    {
