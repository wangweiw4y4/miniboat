...
float thruster_B = 0.2;       // thruster value, B - 1.257
...
int calibration_time = 600;     // 300 * 1/10 (10hz loop rate) = 30 seconds
...
void increment_thruster()
{
    ...
    { // angular velocity is +ive, therefore increase thruster B.
        thruster_B = thruster_B * 1.01;
    }
    else if (angular_v < -threshold)
    { // angular velocity is -ive, therefore decrease thruster B.
        ...
        if (calibration)
        {
            ...
            if (t > initial_accel)
            {
                ...
                else
                {
                    ...
                    if (angular_stability == 30 || t > calibration_time)
                    {                                   // once it's stable, or too much time has passed, call the calibration finished.
