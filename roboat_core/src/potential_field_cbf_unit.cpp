...
#include <roboat_core/potential_field_cbf.h>
...
PotentialField::PotentialField(ros::NodeHandle nh) : nh_(nh)
{
    ...
    number_of_robots = 0;
    counter = 0;
    des_shape = 0; // starts with no active shape being tracked
    // des_shape = 0;
    scale = 1;
    attractive_flag = 2;
    ...
    FUNCTIONS_DIR_DEFAULT = "/home/ubuntu/catkin_ws/src/miniboat/roboat_core/scripts/cbf";
    nh_.param("functions/path", FUNCTIONS_DIR, FUNCTIONS_DIR_DEFAULT);
    ocpC_function = casadi::Function::load(FUNCTIONS_DIR + "/ocpCBF_unit.casadi");
    ...
    C_j = MatrixXd::Zero(2*(swarm_size_-1), 1);
    V_j = MatrixXd::Zero(2*(swarm_size_-1), 1);
...
void PotentialField::timeStep(polygon_t _shape)
{
    ...
    for (int i = 0; i < number_of_robots; i++)
    {
        ...
        if (r <= neighbour_radius){
            ...
            if ((r <= 0.23) && (r >= 0.18) && (attractive_flag == 1)){ //HERE IS THE TRESHOLD FOR DELATCHING
                ...
                ROS_WARN("Delatching %f", desired_psi); // possible alternative: array of flags to check twice if miniboats are latched
            }
    ...
    if (attractive_flag == 1){
        ...
        repulsive_force << 0.0, 0.0; //= srf * Fr + stf * Ftheta;
    }
    //}
    ...
    u_mpc(0,0) = body_force(0);
    v_mpc(0,0) = body_force(1);
    ...
    body_force(0) = u_cbf(0,0); 
    body_force(1) = v_cbf(0,0);    
